...
### 2025Q4以来，使用v100推理新的开源模型已基本失去了生产力，特别是prefill不忍直视。作为8卡v100的拥有者，终于忍无可忍，利用春节假期及之后的业余时间，开发了这个项目，解决了个人之前痛点困扰，快哉！
本项目专为V100开发，基于lmdeploy 0.12.1，支持了2025Q4至2026Q1的个人想用主流开源模型，未考虑兼容其他架构，只在8卡V100 32g设备验证。因工作量巨大，与原项目合并难度很大，所以先独立发布，与同样的v100拥有者同乐。欢迎PR。</br>
支持模型：</br>
Qwen3.5-122B-A10B-AWQ-4bit、Qwen3.5-35B-A3B-AWQ、Qwen3.5-27B-AWQ-4bit、Qwen3.5-9B-AWQ-4bit、Qwen3.5-4B-AWQ-4bit、Qwen3.5-2B、Qwen3.5-0.8B<b>（支持含视频的多模态、mtp）</b></br>
MiniMax-M2.5-AWQ、MiniMax-M2.1-AWQ</br>
Step-3.5-Flash-int4-mixed-AutoRound（支持mtp）、Step-3.5-Flash-AWQ-4bit（该模型未含mtp层）</br>
Qwen3-Coder-Next-AWQ-4bit、Qwen3-Next-80B-A3B-Thinking-AWQ-4bit</br>
新增特性：</br>
anthropic api</br>
autoround、compressed-tensors量化支持</br>
gguf支持（进行中）</br>
v100底层MMA_884重构（进行中）
...
# 3. 安装 PyTorch (CUDA 11.8 示例，根据实际 CUDA 版本调整)
pip install torch torchvision --index-url https://download.pytorch.org/whl/cu118

# 4. 开发模式安装 (自动编译 C++/CUDA)
pip install -e .
...
> 编译需要 CMake 3.11+、Ninja、GCC 支持 C++17。V100 用户确保 `CUDA_DEVICE_ORDER=PCI_BUS_ID` 已设置。

---

## 各模型推理启动命令示例

以下均使用 TurboMind 引擎 (`lmdeploy serve api_server`)，请根据实际模型路径替换。

### Qwen3.5 系列

```bash
# Qwen3.5-122B-A10B-AWQ (MoE, TP=8, 跨NUMA需禁用P2P)
...
# Qwen3.5-35B-A3B-AWQ (MoE, TP=4, NUMA内NVLink)
...
多模态输入通过 OpenAI 兼容 API 的 `image_url` / `video_url` 字段传入，启动命令与上面相同，无需额外参数。

### MiniMax 系列

```bash
# MiniMax-M2.5-AWQ (TP=8, 跨NUMA)
...
```bash
# Step-3.5-Flash AutoRound int4 混合量化 (TP=8, 支持MTP)
