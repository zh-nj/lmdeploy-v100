...
class TurboMind:
    ...
    def _from_hf(self, model_path: str, engine_config: TurbomindEngineConfig):
        ...
        if is_gguf_model(model_path):
            # GGUF path: auto-set model_format and skip is_supported() check
            # (GGUF arch detection is handled by archs.py).
            if engine_config.model_format is None:
                engine_config.model_format = 'gguf'
            logger.info(f'_from_hf: GGUF model detected, format={engine_config.model_format}')
        else:
        ...
        logger.info('_from_hf: calling get_tm_model')
        ...
        logger.info('_from_hf: get_tm_model done')
