...
def get_u8_slices(x: torch.Tensor, dtype: torch.dtype) -> list:
    """Unpack int32 into 8-bit slices (4 values per int32)."""
    MAP = {torch.int32: 4, torch.uint8: 1}
    xs = []
    for _ in range(MAP[x.dtype]):
        xs.append((x & 255).to(dtype))
        x = x >> 8
    return xs


def process_gptq(x: torch.Tensor, kind: str, bits: int = 4):
    """Process GPTQ quantized tensor.

    Args:
        x: raw tensor from safetensors
        kind: weight component type ('qweight', 'qzeros', 'scales')
        bits: quantization bit-width (4 or 8)
    """
    x = x.cuda()
    if x.dtype == torch.int32:
        if bits == 8:
            xs = get_u8_slices(x, torch.uint8)
            if kind == 'qweight':  # (k/4,n) for 8-bit
                x = torch.stack(xs, dim=1).view(-1, x.size(-1))
            else:  # 'qzeros' (k/g,n/4) for 8-bit
                x = torch.stack(xs, dim=-1).view(x.size(0), -1) + 1
        else:
            xs = get_u4_slices(x, torch.uint8)
            if kind == 'qweight':  # (k/8,n) for 4-bit
                x = torch.stack(xs, dim=1).view(-1, x.size(-1))
            else:  # 'qzeros' (k/g,n/8) for 4-bit
                x = torch.stack(xs, dim=-1).view(x.size(0), -1) + 1
    if kind in ['qweight', 'qzeros', 'scales']:
        x = x.t()
    return x
...
def process_gguf(x: torch.Tensor, kind: str):
    """GGUF weights: quantized data is raw uint8, pass through as-is."""
    return x.cuda()
