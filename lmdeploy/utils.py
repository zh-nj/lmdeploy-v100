...
def _get_and_verify_max_len(
    ...
    for key in llm_keys:
        ...
        if nested is not None:
            # Some model types (e.g. qwen3_5_moe) have text_config as a
            # raw dict instead of a PretrainedConfig object.  Convert to
            # a simple namespace so getattr works uniformly below.
            if isinstance(nested, dict):
                from types import SimpleNamespace
                nested = SimpleNamespace(**nested)
            hf_config = nested
            break
