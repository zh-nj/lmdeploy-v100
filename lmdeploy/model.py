...
class HFChatTemplate(BaseChatTemplate):
    ...
    def __init__(self, model_path: str = '', **kwargs):
        ...
        try:
            ...
            # Some models do not support a system-only message.
            # Qwen3.5 requires a user message to be present.
            # Try to extract system/assistant tokens from a combined message.
            try:
                self.sentinel_system_messages, self.sentinel_system_prompt = \
                    self._extract_system_from_combined(model_path)
                self.assistant_start, self.assistant_end = \
                    self._extract_assistant_from_combined()
            except Exception:
                arch, _ = get_model_arch(model_path)
    ...
    def messages2prompt(self, messages, sequence_start=True, **kwargs):
        ...
        # Strip trailing <think>\n or <think>\n\n injected by chat templates
        # (e.g. Qwen3.5). The model should generate <think> on its own;
        # leaving it in the prompt causes some inputs to loop on <think> tokens.
    ...
    def _extract_system_from_combined(self, model_path=None):
        """Extract system sentinel from a system+user combined message.

        Some models (e.g. Qwen3.5) require a user message to be present
        even when testing system role. This method uses a combined message
        to extract the system portion.
        """
        messages = [
        ...
        prompt = self.tokenizer.apply_chat_template(messages, tokenize=False)
        sys_pos = prompt.find('SYSTEM_SENTINEL')
        if sys_pos == -1:
            # Model doesn't include system content — treat as no system support
            return [], ''
        sentinel_system_prompt = prompt[:sys_pos + len('SYSTEM_SENTINEL')]
        # Find where user section starts to get the end of system section
        user_pos = prompt.find('USER_SENTINEL')
        sentinel_system_messages = [{'role': 'system', 'content': 'SYSTEM_SENTINEL'}]
        return sentinel_system_messages, sentinel_system_prompt

    def _extract_assistant_from_combined(self):
        """Extract assistant start/end tokens from a user+assistant message."""
        messages = [
            {'role': 'user', 'content': 'USER_SENTINEL'},
            {'role': 'assistant', 'content': 'ASST_SENTINEL'},
        ]
        prompt = self.tokenizer.apply_chat_template(messages, tokenize=False)
        asst_pos = prompt.find('ASST_SENTINEL')
        if asst_pos == -1:
            raise ValueError('Cannot find assistant sentinel in template')
        # Find the start of assistant section (between user end and assistant content)
        user_pos = prompt.find('USER_SENTINEL')
        after_user = prompt[user_pos + len('USER_SENTINEL'):]
        asst_pos_in_after = after_user.find('ASST_SENTINEL')
        assistant_start_section = after_user[:asst_pos_in_after]
        assistant_end = after_user[asst_pos_in_after + len('ASST_SENTINEL'):]
        return assistant_start_section, assistant_end
