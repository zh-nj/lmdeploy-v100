...
class ArgumentHelper:
    ...
    @staticmethod
    def num_draft_tokens(parser):
        """Add argument num_draft_tokens to parser."""
        return parser.add_argument('--num-draft-tokens',
                                   type=int,
                                   default=0,
                                   help='Number of draft tokens K for MTP speculative decoding. '
                                   'When > 0, speculative decoding is auto-enabled. '
                                   'Default 0 means use the value from model config.')

    @staticmethod
    def speculative_decoding(parser):
        """Add argument speculative_decoding to parser."""
        return parser.add_argument('--speculative-decoding',
                                   action='store_true',
                                   default=False,
                                   help='Enable MTP speculative decoding. '
                                   'Auto-enabled when --num-draft-tokens > 0.')
