...
from lmdeploy.serve.openai.serving_anthropic import (
...
async def messages_v1(request: MessagesRequest, raw_request: Request = None):
    """Anthropic Messages API endpoint.

    Accepts requests in Anthropic Messages API format and translates them
    to the internal engine format for inference.
    """
    # Check model name
    model_name = request.model
    adapter_name = None
    if model_name != VariableInterface.async_engine.model_name:
        ...
        if not hasattr(VariableInterface.async_engine, 'adapter_names') or \
                model_name not in getattr(VariableInterface.async_engine, 'adapter_names', []):
            # Allow adapter_name to pass through — engine will reject if invalid
            pass

    try:
        oai_msgs, gen_config, tools, tool_choice_val = translate_messages_request(request)
    except Exception as e:
        return create_anthropic_error(400, str(e))

    session = VariableInterface.get_session(-1)
    request_id = str(session.session_id)

    if tools and tool_choice_val != 'none':
        gen_config.skip_special_tokens = False

    # Apply tool_choice to tools list if specific tool requested
    filtered_tools = None
    if tools:
        if isinstance(tool_choice_val, str) and tool_choice_val not in ('auto', 'required', None):
            filtered_tools = [t for t in tools if t['name'] == tool_choice_val]
        else:
    ...
    if request.stream:
        return StreamingResponse(
            anthropic_stream_generator(
                ...
                request_id,
                model_name,
                tool_parser=VariableInterface.tool_parser,
                reasoning_parser=VariableInterface.reasoning_parser,
    ...
    async for res in result_generator:
        if raw_request and await raw_request.is_disconnected():
            await session.async_abort()
            return create_anthropic_error(400, 'Client disconnected')
        final_res = res
        text += res.response

    if final_res is None:
        return create_anthropic_error(500, 'No response from engine')

    if final_res.finish_reason == 'error':
        return create_anthropic_error(500, text or 'Internal engine error')

    # Parse tool calls and reasoning
    tool_calls = None
    reasoning_content = None
    if VariableInterface.tool_parser is not None and tools:
        try:
            tool_info = VariableInterface.tool_parser.extract_tool_calls(text, request=None)
            text = tool_info.content or ''
    ...
    if VariableInterface.reasoning_parser is not None:
        reasoning_content, text = VariableInterface.reasoning_parser.extract_reasoning_content(text, None)

    # Detect matched stop sequence (fix #6)
    stop_word_matched = False
    matched_stop_seq = None
    if (request.stop_sequences
            and final_res.finish_reason == 'stop'
            and hasattr(final_res, 'stop_word')
            and final_res.stop_word):
        stop_word_matched = True
        matched_stop_seq = final_res.stop_word
    ...
    return build_messages_response(
        ...
        tool_calls=tool_calls,
        reasoning_content=reasoning_content,
        finish_reason=final_res.finish_reason or 'stop',
...
async def count_tokens_v1(request: CountTokensRequest):
    """Anthropic count_tokens endpoint.

    Returns the input token count for the given messages, processed through
    the chat template for accuracy (matching vLLM behaviour).
    """
    try:
        # Build a minimal MessagesRequest to reuse translation logic
        dummy = MessagesRequest(
            model=request.model,
        ...
        oai_msgs, _, tools, _ = translate_messages_request(dummy)

        # Use chat_template.messages2prompt for accurate token counting
        engine = VariableInterface.async_engine
        chat_template = getattr(engine, 'chat_template', None)
        tokenizer = engine.tokenizer

        if chat_template is not None:
            prompt = chat_template.messages2prompt(
                oai_msgs, sequence_start=True, tools=tools)
            input_ids = tokenizer.encode(prompt, add_bos=False)
        else:
            # Fallback: naive text join + tokenize
            prompt = ''
            for m in oai_msgs:
                c = m.get('content', '')
                if isinstance(c, str):
        ...
        return CountTokensResponse(
            ...
            context_management=AnthropicContextManagement(
                original_input_tokens=n_tokens),
        )
    except Exception as e:
        return create_anthropic_error(400, str(e))
