        ...
        def _inner_call(i, in_messages, out_messages):
            ...
            for item in content:
                ...
                if item['type'] == 'image_url':
                    ...
                    from lmdeploy.vl.utils import load_video
                    data = item['video_url'].copy()
                    try:
                        url = data.pop('url')
                        max_num_frames = data.pop('max_num_frames', None)
                        fps = data.pop('fps', 2.0)
                        frames = load_video(url, max_num_frames=max_num_frames, fps=fps)
                        data.update(type='video', frames=frames)
                        message['content'].append(data)
                    except KeyError:
                        logger.error(f'invalid format {message}')
    ...
    def _has_multimodal_input(self, messages: List[Dict]) -> bool:
        ...
        return any(
            isinstance(message.get('content'), list) and any(
                item.get('type') in ['image_url', 'image_data', 'time_series', 'video_url'] for item in message['content'])
