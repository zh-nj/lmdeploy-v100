...
struct Engine::Impl {
    ...
    // Check if any request in the batch is in decode phase (generating == true)
    static bool HasDecodeRequests(const BatchData& d);

    // Speculative decoding: GDN snapshot → draft → verify → reject → rollback
    void SpeculativeDecode(BatchData& d);
...
void Engine::Impl::CreateSequenceManager()
{
    ...
    // Use local (per-GPU) kv_head_num for block allocation.
    // The attention kernel creates block layout with local_kv_head_num (= kv_head_num / attn_tp_size),
    // so the block allocation must match to avoid wasting (tp-1)/tp of each block's memory.
    const int local_kv_head_num = (int)model_param.kv_head_num / param_.attn_tp_size;
    ...
    // For mixed attention architectures (e.g. GDN + full attention), only full attention layers
    // need KV cache blocks. Count attention layers to avoid wasting block space on non-attention layers.
    // Only count main model layers (layer_num), not MTP layers that may be appended to layer_types.
    size_t attn_layer_count = model_param.layer_num;
    if (!model_param.layer_types.empty()) {
        auto end = model_param.layer_types.begin()
                   + std::min(model_param.layer_types.size(), static_cast<size_t>(model_param.layer_num));
        attn_layer_count = std::count(model_param.layer_types.begin(), end, 0);
        if (attn_layer_count == 0) {
            attn_layer_count = 1;  // at least 1 to avoid zero-size blocks
        }
        TM_LOG_INFO("[CreateSequenceManager] attn_layer_count = %zu / %zu", attn_layer_count, model_param.layer_num);
    }

    // MTP predictor layers each have one full-attention decoder layer that needs its own KV cache.
    // MTP KV cache layer indices are assigned contiguously after main model: N, N+1, ..., N+num_mtp_layers-1
    if (model_param.num_mtp_layers > 0) {
        attn_layer_count += model_param.num_mtp_layers;
        TM_LOG_INFO("[CreateSequenceManager] attn_layer_count with MTP = %zu (added %d MTP layers)",
                    attn_layer_count,
                    model_param.num_mtp_layers);
    }

    seq_mgr_ = std::make_unique<SequenceManager>(attn_layer_count,
                                                 block_config,
...
void Engine::Impl::Schedule()
{
    ...
    // Decode-progress requests are prioritized to reduce decode slowdowns when
    // long-prefill requests overlap with ongoing generations.
    constexpr uint64_t kNonGeneratingPriorityBias = uint64_t{1} << 63;
    ...
    for (int i = 0; i < s.size(); ++i) {
        ...
        if (const auto& c = s.rc[i]) {
            ...
            uint64_t priority = c->req->unique_id;
            if (!c->generating) {
                priority |= kNonGeneratingPriorityBias;
            }
    ...
    // Inject draft tokens for steady-state speculative decoding iterations
    if (param_.speculative_decoding) {
        for (auto i : existing) {
            auto& c = *cache[i];
            if (c.generating && c.num_drafts > 0) {
                // bonus_token is already at c.token_ids[c.seq_len-1] (written by previous Rollback)
                // draft tokens go at c.token_ids[c.seq_len .. c.seq_len + num_drafts - 1]
                for (int k = 0; k < c.num_drafts; ++k) {
                    c.token_ids[c.seq_len + k] = c.draft_tokens[k];
                }
                // Override to prefill-like path for K+1 tokens (bonus + drafts)
                c.autoregres = false;
                // Reset alpha to 0: Materialize already used alpha for block allocation,
                // but InputProcessor::Prepare uses alpha as input offset (skipping tokens).
                // For spec decode, all input tokens start at history_len, not history_len + alpha.
                c.alpha = 0;
            }
        }
    }
...
void Engine::Impl::Setup(BatchData& d)
{
    ...
    // Copy block_ptrs into BatchData for Compute thread's DraftTokens (MTP SetupAttention needs them)
    if (d.spec_decode_enabled) {
        const int total_blocks = block_ptrs_offsets_buf_[d.bsz];
        d.spec_block_ptrs         = {total_blocks, kCPU};
        d.spec_block_ptrs_offsets = {d.bsz + 1, kCPU};
        std::copy_n(block_ptrs_buf_.data(), total_blocks, d.spec_block_ptrs.data());
        std::copy_n(block_ptrs_offsets_buf_.data(), d.bsz + 1, d.spec_block_ptrs_offsets.data());
    }
}
...
void Engine::Impl::Update(BatchData& b, std::vector<Signal>& signals)
{
    ...
    for (int i = 0; i < b.rc.size(); ++i) {
        ...
        if (auto& c = *b.rc[i]; c.seq) {
            if (auto& s = *c.seq; generating[i]) {
                // In steady-state spec decode (spec_decode=true), Rollback already wrote all tokens
                // (bonus + accepted drafts) and updated sequence_length. output_ids[i] is invalid
                // because Generation::Forward was skipped.
                // In first decode or non-spec-decode (spec_decode=false), output_ids[i] is the sampled token.
                if (!b.spec_decode) {
    ...
    if (async_ || param_.speculative_decoding) {
        const int K    = param_.speculative_decoding ? model_.model_param().num_draft_tokens : 0;
        const int size = s.active + s.swapout;
        for (int i = 0; i < size; ++i) {
            auto& c = *s.rc[i];
            if (i < s.active) {
                // In async mode, Schedule(N) has already set c.input_len for the current iteration,
                // so it reflects the next iteration's expected input_len.
                // In sync mode, c.input_len is from THIS iteration's Schedule, so for generating
                // requests (which will decode next), we use 1 instead of c.input_len.
                // For non-generating requests in sync mode, alpha must be 0 because c.input_len
                // reflects the CURRENT chunk size, not the next iteration's expected input.
                // Using the current chunk size as alpha would cause SequenceManager to compute
                // a negative input_len on the next iteration.
                if (async_) {
                    c.alpha = c.input_len + (c.generating ? K : 0);
                    c.beta  = c.generating + (c.generating ? K : 0);
                }
                else {
                    // sync mode (speculative_decoding path)
                    // input_len = seq_len + beta - alpha - cache_len
                    // For generating: cache_len = seq_len - 1, so input_len = 1 + beta - alpha
                    //
                    // Steady-state (has drafts): input_len = 1 + K (bonus + K drafts)
                    //   alpha = 1 + K (reserve for next iteration), beta = 2K + 1
                    // First decode (no drafts): input_len = 1 (normal decode)
                    //   alpha = 1 + K (reserve for next iteration after DraftOnly), beta = K + 1
                    if (c.generating) {
                        c.alpha = 1 + K;
                        c.beta  = (c.num_drafts > 0) ? (2 * K + 1) : (K + 1);
                    }
                    else {
                        c.alpha = 0;
                        c.beta  = 0;
...
bool Engine::Impl::HasDecodeRequests(const BatchData& d)
{
...
void Engine::Impl::SpeculativeDecode(BatchData& d)
{
    // Compute block_ptrs on Engine thread (CPU-side, no NCCL needed).
    // Store in BatchData so the Compute thread can use them.
    const int total_blocks = [&] {
        int n = 0;
        for (int i = 0; i < d.bsz; ++i) {
            n += d.rc[i]->seq->blocks.size();
        }
        return n;
    }();

    d.spec_block_ptrs         = Buffer_<void*>{total_blocks, kCPU};
    d.spec_block_ptrs_offsets = Buffer_<int>{d.bsz + 1, kCPU};

    d.spec_block_ptrs_offsets[0] = 0;
    auto* dst                    = d.spec_block_ptrs.data();
    for (int i = 0; i < d.bsz; ++i) {
        const auto& s                    = *d.rc[i]->seq;
        d.spec_block_ptrs_offsets[i + 1] = d.spec_block_ptrs_offsets[i] + s.blocks.size();
        dst = std::transform(s.blocks.cbegin(), s.blocks.cend(), dst, [&](int block_id) {
            return seq_mgr_->GetBlockPtr(block_id);
        });
    }

    // Flag the batch for speculative decoding — Compute thread will execute the 5-step pipeline
    d.spec_decode = true;
}
...
void Engine::Impl::InternalThreadEntry()
{
    ...
    d->spec_decode_enabled = param_.speculative_decoding;

    for (unsigned i = 1; i < data_.size(); ++i) {
        auto bd = std::make_unique<BatchData>(i);
        bd->spec_decode_enabled = param_.speculative_decoding;
    ...
    while (true) {
        ...
        if (n_active) {
            ...
            // Speculative decoding is now handled by the Compute thread via RunWithDrafts.
            // No second push/pop needed here.
