...
struct RequestCache {
    ...
    int  draft_tokens[8] = {};   // previous round's MTP-produced draft token IDs (max K=8)
    int  num_drafts      = 0;    // number of valid drafts (0 = first decode iteration)
    bool all_accepted    = false; // whether all drafts were accepted last round (affects GDN rollback)
