...
struct SiluClamped {
    ...
    __device__ T operator()(T gate, T up) const noexcept
    {
        ...
        float silu_gate = fdividef(gate_f, 1.f + expf(-gate_f));
        float clamped_silu_gate = fminf(limit, silu_gate);
        
        float up_f = (float)up;
        float clamped_up = fmaxf(-limit, fminf(limit, up_f));
        
        return static_cast<T>(clamped_silu_gate * clamped_up);
    }
};
...
void Activation(Ref<Tensor> gate_, const Tensor& up, float clamp_limit, ActivationType type, cudaStream_t stream)
{
    auto& gate = gate_.get();
    ...
    auto dispatch = [&](auto t) {
        ...
        if (type == ActivationType::kSilu) {
            if (clamp_limit > 0.f) {
                return invoke(t, SiluClamped<T>{clamp_limit});
            }
        ...
        else if (type == ActivationType::kSwigluStep) {
            return invoke(t, SiluClamped<T>{clamp_limit > 0.f ? clamp_limit : 7.f});
        }
        else {
