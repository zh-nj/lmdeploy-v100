                                 ...
                                 bool         use_sigmoid)
{
    ...
    if (use_sigmoid) {
        ...
        for (int i = 0; i < items_per_thread; ++i) {
            ...
            // Do not alter -inf so padded experts are not picked
    ...
    // Initialize shared_exp_id and shared_scales to safe defaults.
    // This handles the case where logits are NaN and no experts are selected.
    for (int i = threadIdx.x; i < max_top_k * tokens_per_cta; i += block_dim) {
        int k = i / tokens_per_cta;
        int t = i % tokens_per_cta;
        if (k < max_top_k) {
            smem.shared_exp_id[k][t] = 0;
            smem.shared_scales[k][t] = 0.f;
        }
    }
    ...
    for (int i = 0; i < k_per_thread; ++i) {
        ...
        float scale     = 0.f;
        if (idx < max_top_k) {
            expert_id = smem.shared_exp_id[idx][bti2];
            scale     = smem.shared_scales[idx][bti2];
        }
                      ...
                      cudaStream_t st)
{
    ...
    auto dispatch = [&] {
        if (experts <= 8) {
            ...
            else if (experts_per_token <= 16) {
                return invoke(_Int<512>, _Int<16>, _Int<16>, _Int<4>);
            }
        }
        return false;
    };
                     ...
                     cudaStream_t st)
{
    ...
    switch (experts_per_token) {
        ...
        case 10:
            return invoke(std::integral_constant<int, 10>{});
        default:
