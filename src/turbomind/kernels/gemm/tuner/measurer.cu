...
std::pair<float, cudaError_t> Measurer::ColdRun(LaunchSpec spec, const Launcher& launcher, cudaStream_t stream)
{
    ...
    if (status == cudaSuccess) {
        ...
        // Non-fatal: the tuner tries many kernels, some may fail on certain configs (e.g. V100).
        // Clear the error state so subsequent kernels can be tested.
        // cudaDeviceSynchronize + cudaGetLastError clears both sync and async errors.
