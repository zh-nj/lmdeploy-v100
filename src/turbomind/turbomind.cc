...
#include "src/turbomind/kernels/gguf/gguf_types.h"
    ...
    data_type_{}, model_param_{}, attn_param_{}, moe_param_{}, engine_param_{}, ffi_ctx_factory_{ffi_ctx_factory}
{
    ...
    else if (model["activation_type"].as<std::string>("") == "swiglustep") {
        model_param_.act_type = ActivationType::kSwigluStep;
    }
    ...
    if (model["swiglu_limits"]) {
        auto limits = model["swiglu_limits"];
        for (auto it = limits.begin(); it != limits.end(); ++it) {
            model_param_.swiglu_limits.push_back(it->as<float>());
        }
    }
    if (model["swiglu_limits_shared"]) {
        auto limits = model["swiglu_limits_shared"];
        for (auto it = limits.begin(); it != limits.end(); ++it) {
            model_param_.swiglu_limits_shared.push_back(it->as<float>());
        }
    }

    // Per-layer attention head counts (for models like Step3p5 with mixed head counts)
    if (model["head_num_per_layer"]) {
        auto head_num_list = model["head_num_per_layer"];
        for (auto it = head_num_list.begin(); it != head_num_list.end(); ++it) {
            model_param_.head_num_per_layer.push_back(it->as<int>());
        }
        if (!model_param_.head_num_per_layer.empty()) {
            TM_LOG_INFO("[Per-Layer HeadNum] %zu layers with per-layer head counts",
                        model_param_.head_num_per_layer.size());
        }
    }

    model_param_.attn_bias         = model["attn_bias"].as<int>(0);
    model_param_.attn_output_gate  = model["attn_output_gate"].as<bool>(false);
    model_param_.attn_output_gate_per_head = model["attn_output_gate_per_head"].as<bool>(false);
    ...
    // GatedDeltaNet (Qwen3-Coder-Next) parameters
    if (model["layer_types"]) {
        auto layer_types_node = model["layer_types"];
        for (auto it = layer_types_node.begin(); it != layer_types_node.end(); ++it) {
            auto type_str = it->as<std::string>();
            if (type_str == "full_attention") {
                model_param_.layer_types.push_back(0);
            } else if (type_str == "linear_attention") {
                model_param_.layer_types.push_back(1);
            } else if (type_str == "sliding_attention") {
                // Sliding attention is standard attention with window_size
                model_param_.layer_types.push_back(0);
            } else {
                TM_CHECK(0) << "Unknown layer_type: " << type_str;
            }
        }
    }
    model_param_.gdn.key_head_dim    = model["linear_key_head_dim"].as<int>(0);
    model_param_.gdn.value_head_dim  = model["linear_value_head_dim"].as<int>(0);
    model_param_.gdn.num_key_heads   = model["linear_num_key_heads"].as<int>(0);
    model_param_.gdn.num_value_heads = model["linear_num_value_heads"].as<int>(0);
    model_param_.gdn.conv_kernel_dim = model["linear_conv_kernel_dim"].as<int>(0);
    ...
    // Per-layer RoPE overrides (for models like Step3p5 with mixed rope params)
    if (attention["rope_theta"] && attention["rope_dim"]) {
        auto theta_list = attention["rope_theta"].as<std::vector<float>>();
        auto dim_list   = attention["rope_dim"].as<std::vector<int>>();
        if (!theta_list.empty() && theta_list.size() == dim_list.size()) {
            attn_param_.rope_per_layer.resize(theta_list.size());

            // Parse optional per-layer rope_type list
            std::vector<std::string> type_list;
            if (attention["rope_type"]) {
                type_list = attention["rope_type"].as<std::vector<std::string>>();
            }

            for (size_t i = 0; i < theta_list.size(); ++i) {
                // Clone global rope and override base + dim
                attn_param_.rope_per_layer[i]      = attn_param_.rope;
                attn_param_.rope_per_layer[i].base  = theta_list[i];
                attn_param_.rope_per_layer[i].dim   = dim_list[i];
                // Override rope type if per-layer type is specified
                if (i < type_list.size() && !type_list[i].empty()) {
                    auto layer_type = GetRoPEType(type_list[i]);
                    attn_param_.rope_per_layer[i].type = layer_type;
                    // When layer uses default RoPE (no scaling), reset factor
                    // to 1.0 to avoid inheriting the global scaling factor
                    // (e.g. llama3 factor=2.0) which would incorrectly scale
                    // the RoPE frequencies via inv_factor.
                    if (layer_type == RopeType::kDefault) {
                        attn_param_.rope_per_layer[i].factor = 1.0f;
                    }
                }
            }
            TM_LOG_INFO("[Per-Layer RoPE] %zu layers with per-layer rope params", theta_list.size());
        }
    }
    ...
    moe_param_.fp32_gate         = model["fp32_gate"].as<bool>(false);
    ...
    model_param_.expert_ggml_type   = gguf::GGML_TYPE_COUNT;  // default: not GGUF

    // Parse per-layer expert weight types (mixed-quantization models like M2.5-AWQ
    // where some layers' experts are fp16 while others are int4).
    if (model["expert_weight_types"] && model["expert_weight_types"].IsSequence()) {
        for (const auto& node : model["expert_weight_types"]) {
            model_param_.expert_weight_types.push_back(data_type_from_string(node.as<std::string>()));
        }
        if (!model_param_.expert_weight_types.empty()) {
            TM_LOG_INFO("[TM] per-layer expert_weight_types: %zu layers, layer 0 = %d",
                        model_param_.expert_weight_types.size(),
                        (int)model_param_.expert_weight_types[0]);
        }
    }

    // Parse per-layer FFN weight types (models where shared expert / dense MLP
    // uses a different precision than the global weight_type, e.g. Step3p5).
    if (model["ffn_weight_types"] && model["ffn_weight_types"].IsSequence()) {
        for (const auto& node : model["ffn_weight_types"]) {
            auto s = node.as<std::string>();
            if (s.empty()) {
                // Empty string means "use global weight_type"
                model_param_.ffn_weight_types.push_back(model_param_.weight_type);
            }
            else {
                model_param_.ffn_weight_types.push_back(data_type_from_string(s));
            }
        }
        if (!model_param_.ffn_weight_types.empty()) {
            TM_LOG_INFO("[TM] per-layer ffn_weight_types: %zu layers, layer 0 = %d",
                        model_param_.ffn_weight_types.size(),
                        (int)model_param_.ffn_weight_types[0]);
        }
    }

    // Parse per-layer expert GGML types (UD models use different types per layer).
    if (model["expert_ggml_type"].IsSequence()) {
        for (const auto& node : model["expert_ggml_type"]) {
            model_param_.expert_ggml_types.push_back(node.as<int>());
        }
        // Set the scalar field to the first non-sentinel value for backward compat.
        for (int v : model_param_.expert_ggml_types) {
            if (v != gguf::GGML_TYPE_COUNT) {
                model_param_.expert_ggml_type = v;
                break;
            }
        }
    }
    else {
        model_param_.expert_ggml_type = model["expert_ggml_type"].as<int>(gguf::GGML_TYPE_COUNT);
    }

    // Parse per-layer expert GGML types for w2 (down_proj) — UD models may differ.
    if (model["expert_ggml_type_w2"] && model["expert_ggml_type_w2"].IsSequence()) {
        for (const auto& node : model["expert_ggml_type_w2"]) {
            model_param_.expert_ggml_types_w2.push_back(node.as<int>());
        }
    }
    ...
    // MTP speculative decoding parameters
    model_param_.num_mtp_layers   = model["num_mtp_layers"].as<int>(0);
    model_param_.num_draft_tokens = model["num_draft_tokens"].as<int>(0);
    model_param_.mtp_expert_weight_type = model["mtp_expert_weight_type"].as<std::string>("");
    model_param_.mtp_has_shared_head   = model["mtp_has_shared_head"].as<bool>(false);

    engine_param_.speculative_decoding = engine["speculative_decoding"].as<bool>(false);

    // Default derivation: when num_mtp_layers >= 1 and num_draft_tokens not explicitly set, use num_mtp_layers
    if (model_param_.num_mtp_layers >= 1 && model_param_.num_draft_tokens == 0) {
        model_param_.num_draft_tokens = model_param_.num_mtp_layers;
        TM_LOG_INFO("[TM] num_draft_tokens not set, defaulting to num_mtp_layers = %d",
                    model_param_.num_draft_tokens);
    }

    // Warning: speculative_decoding enabled but no MTP layers available
    if (engine_param_.speculative_decoding && model_param_.num_mtp_layers == 0) {
        TM_LOG_WARNING("[TM] speculative_decoding is enabled but num_mtp_layers is 0, falling back to standard decoding.");
        engine_param_.speculative_decoding = false;
    }

    // Async double-buffering is incompatible with speculative decoding.
    // In async mode (phases_=2), the engine pops the PREVIOUS iteration's batch after pushing
    // the current one for spec decode, causing Update to process the wrong batch's results.
    // Force synchronous mode (phases_=1) when spec decode is enabled.
    if (engine_param_.speculative_decoding && phases_ > 1) {
        TM_LOG_WARNING("[TM] Disabling async mode (async_=0) because speculative_decoding is enabled.");
        phases_ = 1;
    }
