...
#include "src/turbomind/kernels/gguf/dequantize.h"
                            ...
                            std::optional<Tensor>   output)
{
    ...
    if (weight.is_gguf()) {
        // GGUF path: dequantize raw quantized weight to FP16, then GEMM
        auto st = core::Context::stream().handle();
        const int64_t n_elements = (int64_t)weight.input_dim * weight.output_dim;

        // Allocate temporary FP16 buffer for dequantized weight
        Tensor dequant_w{{weight.output_dim, weight.input_dim}, kFloat16, kDEVICE};

        gguf::dequantize_gguf(weight.ggml_type,
                              weight.gguf_raw_weight.raw_data(),
                              (half*)dequant_w.raw_data(),
                              n_elements,
                              st);

        // Build a temporary dense weight descriptor for the GEMM
        LlamaDenseWeight tmp_dense;
        tmp_dense.input_dim  = weight.input_dim;
        tmp_dense.output_dim = weight.output_dim;
        tmp_dense.data_type  = weight.data_type;
        tmp_dense.weight     = dequant_w;
        tmp_dense.weight_type = kFloat16;
        tmp_dense.input_type  = weight.input_type;
        tmp_dense.epilogue    = weight.epilogue;
        tmp_dense.weight_quant = {};
        tmp_dense.input_quant  = {};
        // GGUF dequant outputs row-major (in_dim, out_dim) — same as
        // the standard emplace() path after Python-side transpose.
        tmp_dense.k_desc = {kFloat16, gemm::Order::kRowMajor,
                            weight.input_dim, weight.output_dim, weight.output_dim};
        tmp_dense.q_desc = {};

        impl_->Forward(out, in, tmp_dense, indices, offsets);
    }
    else {
