...
void UnifiedDecoder::Run(BatchOp op, int phase, TensorMap& env)
{
    ...
    if (gated_delta_layer_) {
        gated_delta_layer_->Run(op, phase, env);
    }
}
    ...
    is_warm_up_{*ctx.is_warm_up}
{
    ...
    // Initialize layer types (default: all full attention)
    layer_types_.resize(model.layer_num, 0);
    if (!model.layer_types.empty()) {
        for (size_t i = 0; i < model.layer_num && i < model.layer_types.size(); ++i) {
            layer_types_[i] = model.layer_types[i];
        }
    }

    // Create GatedDeltaNetLayer if any linear attention layers exist
    bool has_linear_attn = std::any_of(layer_types_.begin(), layer_types_.end(), [](int t) { return t == 1; });
    if (has_linear_attn) {
        gated_delta_layer_ = std::make_unique<GatedDeltaNetLayer>(model, model.gdn, engine, ctx);
    }

    // Build mapping from global layer_id to KV cache attn_layer_index.
    // Only full attention layers (type==0) need KV cache blocks.
    // For models without layer_types (all full attention), this is identity mapping.
    attn_layer_indices_.resize(model.layer_num, -1);
    int attn_idx = 0;
    for (size_t i = 0; i < model.layer_num; ++i) {
        if (layer_types_[i] == 0) {
            attn_layer_indices_[i] = attn_idx++;
        }
    }
}
...
void UnifiedDecoder::Forward(int phase, TensorMap& args, const std::vector<WeightType*>& weights)
{
    ...
    for (int layer = 0; layer < layer_num_; ++layer) {
        ...
        TM_LOG_DEBUG("Layer %d: type=%d", layer, layer_types_[layer]);
        if (layer_types_[layer] == 1 && gated_delta_layer_) {
            // Linear attention (GatedDeltaNet)
            gated_delta_layer_->Forward(
                {phase, local_hidden_states, local_hidden_states, weights.at(layer)->linear_attn_weights.get(), layer});
        }
        else {
            // Full attention — use mapped attn_layer_index for KV cache block indexing
            const int attn_layer_idx = attn_layer_indices_[layer];
            attn_layer_->Forward(
                {phase, local_hidden_states, local_hidden_states, weights.at(layer)->self_attn_weights.get(), attn_layer_idx});
        }

        TM_DEBUG_TENSOR(local_hidden_states, Concat("attn_block", layer), 2);

        // Get output bias from the appropriate attention weight
        const Tensor& output_bias = (layer_types_[layer] == 1 && weights.at(layer)->linear_attn_weights)
                                        ? weights.at(layer)->linear_attn_weights->out_proj.bias
    ...
    // Produce un-normed residual for MTP speculative decoding BEFORE the Copy below
    // overwrites local_residual with normed hidden_states.
    if (args.try_("output_last_residual")) {
        Tensor selected_residual;
        if (reuse_hidden_states) {
            // Need an independent copy because local_residual will be overwritten below.
            // Use explicit cudaMemcpyAsync to avoid stack allocator issues.
            selected_residual = {{local_token_num, (int)hidden_units_}, dtype, kDEVICE};
            cudaMemcpyAsync(selected_residual.raw_data(),
                            local_residual.raw_data(),
                            byte_size(dtype, (size_t)local_token_num * hidden_units_),
                            cudaMemcpyDeviceToDevice,
                            stream);
        }
        else {
            selected_residual = {{selected_pos.size(), (int)hidden_units_}, dtype, kDEVICE};
            CollectHiddenStates(local_residual, selected_pos, selected_residual, stream);
        }
        args.produce("last_residual", selected_residual);
    }
...
void UnifiedDecoder::SnapshotGDNState(cudaStream_t stream)
{
    if (gated_delta_layer_) {
        gated_delta_layer_->SnapshotState(stream);
    }
}

void UnifiedDecoder::RestoreGDNState(cudaStream_t stream)
{
    if (gated_delta_layer_) {
        gated_delta_layer_->RestoreState(stream);
    }
}

void UnifiedDecoder::DiscardGDNSnapshot()
{
    if (gated_delta_layer_) {
        gated_delta_layer_->DiscardSnapshot();
    }
}

void UnifiedDecoder::SwapGDNStateAndSnapshot(cudaStream_t stream)
{
    if (gated_delta_layer_) {
        gated_delta_layer_->SwapStateAndSnapshot(stream);
    }
}
