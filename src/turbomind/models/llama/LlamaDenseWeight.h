...
struct LlamaDenseWeight: public core::Module {
    ...
    /// Allocate a GGUF raw-weight buffer instead of FP16 weight tensor.
    /// The buffer stores quantized data in GGUF format; dequantization
    /// happens at GEMM time in LlamaLinear::Forward.
    void emplace_gguf(int input_dim, int output_dim, DataType data_type, bool bias, gguf::GGMLType gtype);
    ...
    {
        return static_cast<bool>(weight) || is_gguf();
    }
    ...
    // GGUF quantized weight support
    gguf::GGMLType ggml_type = gguf::GGML_TYPE_COUNT;  // GGML_TYPE_COUNT = not GGUF
    Tensor         gguf_raw_weight;                     // raw quantized data on GPU

    bool is_gguf() const noexcept { return ggml_type != gguf::GGML_TYPE_COUNT; }
};
...
struct LlamaAttentionWeight: public core::Module {
    ...
    LlamaDenseWeight gate;  // attention output gate (Qwen3-Next: output *= sigmoid(gate))
...
struct LlamaFfnWeight: core::Module {
    ...
    /// GGUF variant: allocates raw quantized buffers instead of FP16.
    LlamaFfnWeight(int            hidden_dim,
                   int            inter_size,
                   bool           bias,
                   int            tp_size,
                   ...
                   ActivationType act_type,
                   bool           fuse_silu_act,
                   gguf::GGMLType gtype_w2 = gguf::GGML_TYPE_COUNT,
                   float          swiglu_limit = 0.0f);
...
struct MoeFfnWeight: core::Module {
    ...
    MoeFfnWeight(int             layer_id,
                 ...
                 ActivationType  act_type,
                 bool            fuse_silu_act,
                 gguf::GGMLType  expert_ggml_type = gguf::GGML_TYPE_COUNT,
                 gguf::GGMLType  expert_ggml_type_w2 = gguf::GGML_TYPE_COUNT,
                 float           swiglu_limit = 0.0f,
                 float           swiglu_limit_shared = 0.0f);
