...
// Simple argmax kernel: one block per row, each block finds the argmax of vocab_size elements.
// Uses shared memory reduction. Each thread handles multiple elements via striding.
template<typename T>
__global__ void ArgmaxKernel(int* output_ids, const T* logits, int vocab_size)
{
    const int row = blockIdx.x;
    const T*  row_logits = logits + (size_t)row * vocab_size;
    ...
    if (threadIdx.x == 0) {
        output_ids[row] = max_idx;
    }
}

void invokeArgmax(int* output_ids, const void* logits, int batch_size, int vocab_size, DataType dtype, cudaStream_t st)
{
    constexpr int block = 256;
    const int     grid  = batch_size;
    if (dtype == DataType::kFloat16) {
        ArgmaxKernel<<<grid, block, 0, st>>>(output_ids, (const half*)logits, vocab_size);
    }
#ifdef ENABLE_BF16
    else if (dtype == DataType::kBfloat16) {
        ArgmaxKernel<<<grid, block, 0, st>>>(output_ids, (const __nv_bfloat16*)logits, vocab_size);
    }
#endif
    else {
        // Fallback: treat as float
        ArgmaxKernel<<<grid, block, 0, st>>>(output_ids, (const float*)logits, vocab_size);
    }
}

__global__ void BuildSpecSelectedPosKernel(int* output, const int* q_offsets, int bsz, int K)
{
    // Each thread handles one (request, draft_index) pair
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = K + 1;
    if (idx >= bsz * stride) return;
    const int i = idx / stride;  // request index
    const int j = idx % stride;  // position within request
    output[idx] = q_offsets[i] + j;
}

void invokeBuildSpecSelectedPos(int* output, const int* q_offsets, int bsz, int K, cudaStream_t st)
{
    const int total = bsz * (K + 1);
    constexpr int block = 256;
    const int grid = (total + block - 1) / block;
    BuildSpecSelectedPosKernel<<<grid, block, 0, st>>>(output, q_offsets, bsz, K);
}

// Gather hidden states: dst[i,:] = src[i*(K+1)+num_accepted[i],:]
// Each block handles one row (one request)
template<typename T>
__global__ void GatherHiddenByAcceptedKernel(T* dst, const T* src, const int* num_accepted, int K, int hidden_size)
{
    const int i = blockIdx.x;
    const int src_row = i * (K + 1) + num_accepted[i];
    const T* src_ptr = src + (size_t)src_row * hidden_size;
    T* dst_ptr = dst + (size_t)i * hidden_size;
    for (int j = threadIdx.x; j < hidden_size; j += blockDim.x) {
        dst_ptr[j] = src_ptr[j];
    }
}

void invokeGatherHiddenByAccepted(
    void* dst, const void* src, const int* num_accepted, int bsz, int K, int hidden_size, DataType dtype, cudaStream_t st)
{
    constexpr int block = 256;
    const int grid = bsz;
    if (dtype == DataType::kFloat16) {
        // Use uint4 (16 bytes = 8 half) for coalesced access
        TM_CHECK(hidden_size % 8 == 0);
        const int vec_size = hidden_size / 8;
        GatherHiddenByAcceptedKernel<<<grid, block, 0, st>>>(
            (uint4*)dst, (const uint4*)src, num_accepted, K, vec_size);
    }
    else {
        // Fallback: element-wise
        GatherHiddenByAcceptedKernel<<<grid, block, 0, st>>>(
            (float*)dst, (const float*)src, num_accepted, K, hidden_size);
    }
}

// Scatter rows: dst[(N*i + t) * row_elems .. +row_elems] = src[i * row_elems .. +row_elems]
// row_size_bytes is in bytes; we use uint4 (16 bytes) for coalesced access.
__global__ void ScatterRowsKernel(char* dst, const char* src, int N, int t, int row_size_bytes)
{
    const int i = blockIdx.x;
    const size_t src_off = (size_t)i * row_size_bytes;
    const size_t dst_off = (size_t)(N * i + t) * row_size_bytes;
    // Copy in 16-byte chunks
    const int n16 = row_size_bytes / 16;
    const uint4* s = (const uint4*)(src + src_off);
    uint4*       d = (uint4*)(dst + dst_off);
    for (int j = threadIdx.x; j < n16; j += blockDim.x) {
        d[j] = s[j];
    }
    // Handle remainder bytes (if row_size_bytes not multiple of 16)
    const int rem_start = n16 * 16;
    for (int j = rem_start + threadIdx.x; j < row_size_bytes; j += blockDim.x) {
        ((char*)dst + dst_off)[j] = ((const char*)src + src_off)[j];
    }
}

void invokeScatterRows(void* dst, const void* src, int bsz, int N, int t, int row_size_bytes, cudaStream_t st)
{
    if (bsz == 0) return;
    constexpr int block = 256;
    ScatterRowsKernel<<<bsz, block, 0, st>>>((char*)dst, (const char*)src, N, t, row_size_bytes);
}

// Build cu_k_len for multi-token decode: new_cu_k_len[i] = k_offsets_orig[i] - i * delta
__global__ void BuildMtdCuKLenKernel(int* cu_k_len_out, const int* k_offsets_orig, int bsz_plus_1, int delta)
{
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < bsz_plus_1) {
        cu_k_len_out[i] = k_offsets_orig[i] - i * delta;
    }
}

void invokeBuildMtdCuKLen(int* cu_k_len_out, const int* k_offsets_orig, int bsz, int delta, cudaStream_t st)
{
    const int n = bsz + 1;
    constexpr int block = 128;
    const int grid = (n + block - 1) / block;
    BuildMtdCuKLenKernel<<<grid, block, 0, st>>>(cu_k_len_out, k_offsets_orig, n, delta);
}

__global__ void IotaKernel(int* output, int n)
{
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) output[i] = i;
}
...
void invokeIota(int* output, int n, cudaStream_t st)
{
    constexpr int block = 128;
    const int grid = (n + block - 1) / block;
    IotaKernel<<<grid, block, 0, st>>>(output, n);
}
