...
void LlamaDenseWeight::emplace_gguf(
    int input_dim, int output_dim, DataType data_type, bool bias, gguf::GGMLType gtype)
{
    this->data_type   = data_type;
    this->input_type  = data_type;
    this->weight_type = data_type;  // GEMM uses dequantized FP16
    this->input_dim   = input_dim;
    this->output_dim  = output_dim;
    this->group_size  = 1;
    this->ggml_type   = gtype;

    // Compute raw byte size: n_elements / elements_per_block * block_byte_size
    const int64_t n_elements  = (int64_t)input_dim * output_dim;
    const int     elems_per_b = gguf::ggml_type_elements_per_block(gtype);
    const size_t  block_bytes = gguf::ggml_type_block_size(gtype);
    TM_CHECK(elems_per_b > 0 && block_bytes > 0) << "unsupported GGUF type " << (int)gtype;
    TM_CHECK(n_elements % elems_per_b == 0)
        << n_elements << " not divisible by block size " << elems_per_b;
    const int64_t n_blocks  = n_elements / elems_per_b;
    const int64_t raw_bytes = n_blocks * (int64_t)block_bytes;

    // Allocate a flat uint8 buffer for the raw quantized data.
    gguf_raw_weight = Tensor{{raw_bytes}, kUint8, kDEVICE};
    register_parameter("weight", gguf_raw_weight);
    ...
    // k_desc / q_desc are not used for GGUF path (LlamaLinear builds a
    // temporary dense weight after dequantization), but set sane defaults
    // matching the dequant output layout: row-major (in_dim, out_dim).
    k_desc = {};
    q_desc = {};
    k_desc.type  = kFloat16;
    k_desc.order = gemm::kRowMajor;
    k_desc.rows  = input_dim;
    k_desc.cols  = output_dim;
    k_desc.ld    = output_dim;
...
void LlamaDenseWeight::prepare(bool fused_moe)
{
    ...
    // Skip conversion for fp32 weights (e.g. fp32 MoE gate) — these are
    // used directly via cuBLAS SGEMM, not through the custom GEMM kernel.
    if (weight_type == kFloat) {
        return;
    }
...
LlamaAttentionWeight::LlamaAttentionWeight(int      hidden_dim,
                                           ...
                                           bool     attn_output_gate_per_head,
                                           int      tp_size,
                                           int      tp_rank,
                                           DataType data_type,
                                           ...
                                           bool     sink)
{
    ...
    if (attn_output_gate) {
        // Gate weight dimensions depend on gate type:
        // per-head scalar (Step3p5): hidden_dim -> num_heads / tp
        // per-element (Qwen3-Next): hidden_dim -> num_heads * head_dim / tp
        const int gate_out_dim = attn_output_gate_per_head ? (head_num / tp_size) : (head_num * head_dim / tp_size);
        gate.emplace(hidden_dim, gate_out_dim, data_type, false, data_type, 0);
        register_module("w_gate", gate, tp_rank);
    }
                               ...
                               float          swiglu_limit)
{
    ...
    // w1 (gate) and w3 (up): [hidden_dim, inter_size] — TP split on
    // output_dim is contiguous in GGUF row-major layout → store as GGUF raw.
    gating.emplace_gguf(hidden_dim, inter_size, data_type, bias, gtype);
    intermediate.emplace_gguf(hidden_dim, inter_size, data_type, bias, gtype);

    // w2 (down): [inter_size, hidden_dim] — TP split on input_dim (rows).
    // Use gtype_w2 if specified (UD models may use different type for w2).
    gguf::GGMLType w2_type = (gtype_w2 != gguf::GGML_TYPE_COUNT) ? gtype_w2 : gtype;
    output.emplace_gguf(inter_size, hidden_dim, data_type, bias, w2_type);
...
{
    // GGUF experts: skip all preprocessing (dequant happens at GEMM time)
    if (gating.is_gguf()) {
        return;
    }
...
MoeFfnWeight::MoeFfnWeight(int             layer_id,
                           ...
                           ActivationType  act_type,
                           bool            fuse_silu_act,
                           gguf::GGMLType  expert_ggml_type,
                           gguf::GGMLType  expert_ggml_type_w2,
                           float           swiglu_limit,
                           float           swiglu_limit_shared)
{
    ...
    // For fp32_gate, store gate weight as fp32 for higher precision routing.
    // data_type is set to kFloat so that weight loading reads .weight as fp32.
    const auto gate_dtype = param.fp32_gate ? kFloat : data_type;
    gate.emplace(hidden_dim, expert_num, gate_dtype, param.router_bias, gate_dtype, 1);
    register_module("gate", gate);

    method = param.method;
    
    this->swiglu_limit = swiglu_limit;
    this->swiglu_limit_shared = swiglu_limit_shared;

    const bool use_gguf = expert_ggml_type != gguf::GGML_TYPE_COUNT;

    const bool is_cublas_gemm = !use_gguf && method == MoeParam::kNaive && byte_size(weight_type, 8) == 16;
...
void MoeFfnWeight::prepare()
{
    ...
    // Check if experts use GGUF raw weights
    const bool is_gguf = !experts.empty() && experts[0]->gating.is_gguf();
    ...
    if (is_gguf) {
        // GGUF experts: skip LinkExperts (no batched GEMM pointers needed).
        // The MoE dispatch will use per-expert LlamaLinear::Forward with
        // on-the-fly dequantization.
        // Force kNaive method for GGUF.
        method = MoeParam::kNaive;
        auto& e = *experts.at(0);
        block.inter_size    = e.inter_size;
        block.is_fused_silu = e.is_fused_silu;
        block.act_type      = e.act_type;
        block.swiglu_limit  = swiglu_limit;
        return;
    }
